...
public class ChoiceFunction extends HyperHeuristicSelector {
    ...
    public ChoiceFunction(double alpha, ArrayList<AlgorithmHH> algs, int numObj, int evaluationsPerTime, int populationsize, double[][] referencePoint) {
        super(algs,numObj, evaluationsPerTime, populationsize, referencePoint);
        this.alpha=alpha;
    }
    ...
    public AlgorithmHH chooseAlg() {
        ...
        for(int i=0; i < algs.size(); i++){
            ...
            AlgorithmRanking rniRanking = this.getMetric(alg, this.hypervolume); //RNI
    ...
    protected double cFunction(AlgorithmHH alg, int freqrank, int rnirank) {
        return this.f1(this.algs.size(), freqrank, rnirank)*this.alpha + alg.getEstimatedTime();
    }
